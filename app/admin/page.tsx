...
import type { Database, Confession, Profile } from "@/lib/supabase/types";
...
export default function AdminPage() {
  ...
  if (!profile) {
    return (
      <main className="cg-admin-shell">
        <section className="cg-admin-card">
          <Link href="/" className="cg-admin-back">← Ledger</Link>
          <div className="cg-eyebrow">Chithraguptha · Steward</div>
          <h1>Moderation desk</h1>
          <p className="cg-admin-copy">This private dashboard controls whether new confessions are published immediately or held for review. It is intentionally separate from the public Soul experience.</p>
          <form onSubmit={login} className="cg-auth-form">
            <label>ADMIN EMAIL<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="admin@example.com" /></label>
            <label>PASSWORD<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required /></label>
            <button className="cg-primary" disabled={busy}>{busy ? <Loader2 className="cg-spin" size={15} /> : <ShieldCheck size={15} />} Enter moderation desk</button>
          </form>
          {message && <div className="cg-admin-message">{message}</div>}
        </section>
  ...
  return (
    <main className="cg-admin-shell">
      <section className="cg-admin-wrap">
        <header className="cg-admin-header">
          <div><div className="cg-eyebrow">Private stewardship</div><h1>Moderation desk</h1><p>Control the Ledger without changing the public experience.</p></div>
          <div className="cg-admin-actions"><Link href="/" className="cg-secondary"><Eye size={14} /> View site</Link><button className="cg-secondary" onClick={logout}><LogOut size={14} /> Sign out</button></div>
        </header>

        <section className="cg-admin-control">
          <div><span className="cg-eyebrow">Publishing gate</span><h2>Moderation is {moderation ? "ON" : "OFF"}</h2><p>{moderation ? "New user confessions become pending until a steward approves them." : "New user confessions publish immediately. You can turn this on at any time."}</p></div>
          <button className={moderation ? "cg-danger" : "cg-primary"} onClick={toggleModeration} disabled={busy}>{moderation ? "Turn moderation off" : "Turn moderation on"}</button>
        </section>

        <div className="cg-admin-stats"><div><strong>{pending.length}</strong><span>Pending</span></div><div><strong>{published.length}</strong><span>Published</span></div><div><strong>{all.length}</strong><span>Total visible</span></div></div>

        <section className="cg-admin-section">
          <div className="cg-eyebrow">Review queue</div>
          <h2>Pending confessions</h2>
          {pending.length === 0 ? <div className="cg-admin-empty"><Check size={18} /> Nothing is waiting for review.</div> : <div className="cg-admin-list">{pending.map((item) => <article className="cg-admin-item" key={item.id}><div className="cg-meta"><span className="cg-pill">{item.category}</span><span className="cg-time">{item.language} · {item.region}</span></div><p>{item.content}</p><small>Soul {item.display_soul || "anonymous"} · {new Date(item.created_at).toLocaleString()}</small><div className="cg-admin-item-actions"><button className="cg-primary" onClick={() => moderate(item.id, "published")}><Check size={14} /> Publish</button><button className="cg-danger" onClick={() => moderate(item.id, "rejected")}><X size={14} /> Reject</button><button className="cg-secondary" onClick={() => remove(item.id)}><Flame size={14} /> Remove</button></div></article>)}</div>}
        </section>
