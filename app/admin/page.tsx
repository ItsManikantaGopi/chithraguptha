...
export default function AdminPage() {
  ...
  async function bootstrap() {
    ...
    if (!data.user) return;
    const { data: rawProfile, error } = await supabase.from("profiles").select("id,soul_id,role,language,region,created_at").eq("id", data.user.id).maybeSingle();
    if (error) { setMessage(error.message); return; }
    const p = rawProfile as AdminProfileRow | null;
    if (p?.role === "admin") { setProfile(p as Profile); await loadData(); }
    else if (p) setMessage("This Soul is not an administrator.");
  }

  async function loadData() {
    const [{ data: rows, error: rowsError }, { data: rawSetting, error: settingError }] = await Promise.all([
      supabase.from("confessions").select("id,soul_id,display_soul,is_seed,language,region,category,content,status,created_at,updated_at,moderated_at,moderated_by").order("created_at", { ascending: false }).limit(200),
  ...
  async function login(event: FormEvent) {
    ...
    if (error || !data.user) setMessage(error?.message || "Could not sign in."); else await bootstrap();
    setBusy(false);
  }

  async function toggleModeration() {
    setBusy(true); const next = !moderation;
    const { error } = await supabase.from("app_settings").update({ moderation_enabled: next, updated_at: new Date().toISOString() }).eq("id", true);
    if (error) setMessage(error.message); else { setModeration(next); setMessage(next ? "Moderation is ON. New confessions will enter the queue." : "Moderation is OFF. New confessions publish immediately."); }
    setBusy(false);
  }
  ...
  async function remove(id: string) {
    ...
    setBusy(true); const { error } = await supabase.from("confessions").delete().eq("id", id);
    if (error) setMessage(error.message); else await loadData();
    setBusy(false);
  }

  async function logout() {
    await supabase.auth.signOut(); setProfile(null); setAll([]);
  }
