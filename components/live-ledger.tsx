...
export default function LiveLedger() {
  ...
  async function loadProfile() {
    ...
    const { data } = await supabase.from("profiles").select("id,soul_id,role,language,region,created_at").eq("id", userData.user.id).maybeSingle();
    setProfile((data as Profile | null) ?? null);
    if (data?.language && copy[data.language as ProductLanguage]) setLanguage(data.language as ProductLanguage);
  ...
  async function load() {
    ...
    const [{ data, error: feedError }, { data: reactionRows, error: reactionError }] = await Promise.all([
      supabase.from("confessions").select("id,soul_id,display_soul,language,region,category,content,status,created_at,updated_at,moderated_at,moderated_by").eq("status", "published").order("created_at", { ascending: false }).limit(60),
  ...
  async function record() {
    ...
    setSaving(true);
    const { data, error: insertError } = await supabase.from("confessions").insert({
      soul_id: profile.id,
      language,
      region,
      ...
      content: text.trim(),
      status: "published",
    }).select("id,soul_id,display_soul,language,region,category,content,status,created_at,updated_at,moderated_at,moderated_by").single();
    setSaving(false);
    if (insertError) {
    ...
    setText("");
    setCategory(0);
    if (data?.status === "pending") notify("Your confession is in the Ledger queue for review.");
    else notify("Your deed has been recorded anonymously.");
    await load();
  }
  ...
  async function react(confessionId: string, type: ReactionType) {
    ...
    setVoted((v) => ({ ...v, [confessionId]: type }));
    setCounts((c) => ({ ...c, [confessionId]: { punya: c[confessionId]?.punya || 0 + (type === "punya" ? 1 : 0), paapa: c[confessionId]?.paapa || 0 + (type === "paapa" ? 1 : 0) } }));
  ...
  return <div className="cg-shell">
    <header className="cg-header">
      ...
      <div className="cg-header-tools">
        ...
        <label className="cg-language"><span>Region</span><select value={region} onChange={(e) => setRegion(e.target.value)} aria-label="Region">{regions.map((r) => <option key={r}>{r}</option>)}</select></label>
    ...
    <main className="cg-main">
      ...
      <section className="cg-stats"><div className="cg-stat"><b>{confessions.length.toLocaleString()}</b><span>Published entries</span></div><div className="cg-stat"><b>{Object.values(counts).reduce((a, c) => a + c.punya + c.paapa, 0).toLocaleString()}</b><span>Judgments</span></div><div className="cg-stat"><b>{profile ? "SOUL" : "OPEN"}</b><span>{profile ? "Authenticated" : "Public preview"}</span></div></section>
      <section className={`cg-composer ${profile ? "" : "cg-preview"}`} id="composer"><div className="cg-composer-heading"><div><div className="cg-eyebrow">{t.recordEyebrow}</div><h2 className="cg-section-title">{t.confession}</h2></div><span className="cg-seal">{profile ? "PRIVATE" : "PREVIEW"}</span></div>{profile ? <><label className="cg-composer-label">CONFIDENTIAL</label><textarea className="cg-textarea" value={text} onChange={(e) => setText(e.target.value.slice(0, 500))} maxLength={500} placeholder={t.placeholder}/><div className="cg-counter">{text.length} / 500</div><div className="cg-compose-row"><select className="cg-select" value={category} onChange={(e) => setCategory(Number(e.target.value))}>{cats.map((c, i) => <option key={i}>{c}</option>)}</select><button className="cg-primary" onClick={record} disabled={saving}>{saving ? <><Loader2 size={15} className="cg-spin"/>Recording…</> : <>{t.record}<ArrowRight size={15}/></>}</button></div><p className="cg-note cg-composer-note"><ShieldCheck size={13}/> Your confession is tied to your anonymous Soul ID, never a real-world identity.</p></> : <><Link href="/login" className="cg-preview-field"><span>{t.placeholder}</span><small>Login or create a Soul to write in the Ledger.</small></Link><div className="cg-counter">0 / 500</div><div className="cg-compose-row"><Link href="/login" className="cg-select cg-preview-control">Choose a category</Link><Link href="/login" className="cg-primary">{t.record}<ArrowRight size={15}/></Link></div></>}</section>
      ...
      {loading ? <div className="cg-mini" style={{ textAlign: "center", padding: 28 }}><Loader2 className="cg-spin"/> Loading the Ledger…</div> : <div className="cg-feed">{confessions.map((item) => { const c = counts[item.id] || { punya: 0, paapa: 0 }; const choice = voted[item.id]; return <article className="cg-card" id={`soul-${item.id}`} key={item.id}><div className="cg-meta"><span className="cg-pill">{item.category}</span><span className="cg-time">{elapsed(item.created_at)} · {item.region}</span></div><div className="cg-confession">{item.content}</div><div className="cg-soul">Soul {displaySoul(item)}</div><div className="cg-reactions"><button className={`cg-react green ${choice === "punya" ? "selected" : ""}`} disabled={!!choice} onClick={() => react(item.id, "punya")}><Leaf size={16}/><b>Punya</b><span>{c.punya}</span></button><button className={`cg-react red ${choice === "paapa" ? "selected" : ""}`} disabled={!!choice} onClick={() => react(item.id, "paapa")}><Flame size={16}/><b>Paapa</b><span>{c.paapa}</span></button></div>{!profile && <div className="cg-micro">Login or create a Soul to pass judgment.</div>}</article>})}</div>}
      <footer className="cg-footer">{t.company}</footer>
    </main>
    {toast && <div className="cg-toast">{toast}</div>}
    <style>{`.cg-soul-header{display:flex;align-items:center;gap:6px;height:34px;padding:0 8px;border:1px solid #66552f;border-radius:9px;background:#15120d;color:#dfc889}.cg-soul-header strong{font:700 10px ui-monospace,monospace;letter-spacing:.08em}.cg-soul-header button{display:grid;place-items:center;width:24px;height:24px;background:transparent;border:0;color:#bca86f}.cg-login-button{height:34px;display:inline-flex;align-items:center;gap:6px;padding:0 11px;border:1px solid #66552f;border-radius:9px;background:#17130d;color:#dfc889;text-decoration:none;font-size:10px;font-weight:800}.cg-nav{display:flex;gap:6px}.cg-nav a{display:inline-flex;align-items:center;padding:8px 11px;border:1px solid #292b31;border-radius:9px;background:#0e1116;color:#9d988f;text-decoration:none;font-size:10px}.cg-nav a.active,.cg-nav a:hover{color:#eed49a;border-color:#66552f;background:#19150e}.cg-spin{animation:cgspin 1s linear infinite}@keyframes cgspin{to{transform:rotate(360deg)}}.cg-preview-field{display:flex;flex-direction:column;justify-content:center;min-height:145px;width:100%;padding:15px;border:1px solid #292b31;border-radius:12px;background:#0b0e12;color:#918c83;text-decoration:none;line-height:1.7}.cg-preview-field small{margin-top:6px;color:#5f5b54;font-size:10px}.cg-preview-control{text-decoration:none;text-align:left;display:flex;align-items:center}.cg-source-banner code{font-family:ui-monospace,monospace;color:#d6b46a}@media(max-width:900px){.cg-header-tools{width:100%}.cg-nav{order:4;width:100%}.cg-nav a{flex:1;justify-content:center}}@media(max-width:560px){.cg-soul-header{width:100%;justify-content:center}.cg-header-tools{gap:7px}.cg-language{flex:1}.cg-language span{display:none}.cg-nav{overflow:auto}.cg-nav a{white-space:nowrap}.cg-login-button{flex:1;justify-content:center}}`}</style>
  </div>;
}
