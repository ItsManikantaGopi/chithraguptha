  ...
  return (
    <main className="cg-auth-shell">
      <section className="cg-auth-card">
        ...
        <h1>{mode === "create" ? "Keep your Soul" : "Login to your Soul"}</h1>
        ...
        <div className="cg-auth-tabs">
          <button type="button" className={mode === "create" ? "active" : ""} onClick={() => { setMode("create"); setMessage(""); setCreated(""); setSoulId(""); }}>Create Soul</button>
        ...
        <form onSubmit={submit} className="cg-auth-form">
          ...
          {mode === "login" && <label>SOUL ID<input value={soulId} onChange={(e) => setSoulId(e.target.value.toUpperCase())} placeholder="A7F9K" maxLength={5} autoCapitalize="characters" required /></label>}
          ...
          <button className="cg-primary" type="submit" disabled={busy}>{busy ? <><Loader2 size={16} className="cg-spin" /> {mode === "create" ? "Creating…" : "Verifying…"}</> : <><KeyRound size={16} />{mode === "create" ? "Generate my Soul ID" : "Login"}</>}</button>
        </form>

        {created && mode === "login" && <div className="cg-soul-preview"><span className="cg-eyebrow">Your anonymous identity</span><strong>#{created}</strong><small>Your five-character Soul ID stays the same across devices when you log in with the same password.</small></div>}
